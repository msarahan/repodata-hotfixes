...
def _replace_vc_features_with_vc_pkg_deps(fn, record, key, instructions):
    python_vc_deps = {
        '2.6': 'vc 9.*',
        '2.7': 'vc 9.*',
        '3.3': 'vc 10.*',
    ...
    if record_name == 'python':
        ...
        if vc_version:
            instructions[key][fn]['features'] = record.get('features') or ""
        ...
        if vc_version:
            ...
            # add a vc dependency
            if not any(d.startswith('vc') for d in record['depends']):
                instructions[key][fn]['depends'] = record["depends"] + ['vc %d.*' % vc_version]
    ...
    for key in ('packages', 'packages.conda'):
        ...
        for fn, record in index.items():
            ...
            if fn.startswith("numba-0.36.1") and record.get('timestamp') != 1512604800000:
                ...
                instructions[key][fn]['timestamp'] = 1512604800000
            ...
            if blas_req_feature:
                ...
                # instructions[key][fn]["requires_features"] = record["requires_features"]

            if record.get("track_features"):
                for feat in record["track_features"].split():
                    if feat.startswith(("rb2", "openjdk")):
                        xtractd = record["track_features"] = _extract_track_feature(record, feat)
                        instructions[key][fn]["track_features"] = xtractd
            ...
            if record['name'] == 'conda-env':
                if not any(d.startswith('python') for d in record['depends']):
                    instructions[key][fn]["namespace"] = "python"
            ...
            if fn == 'cupti-9.0.176-0.tar.bz2':
                ...
                instructions[key][fn]["depends"] = ['cudatoolkit 9.0.*']
            ...
            if subdir.startswith("win-"):
                _replace_vc_features_with_vc_pkg_deps(fn, record, key, instructions)
                ...
                if subdir.startswith("linux-ppc64le"):
                    ...
                    if fn == 'blas-1.0-openblas.tar.bz2':
                        instructions[key][fn]["build_number"] = 7
